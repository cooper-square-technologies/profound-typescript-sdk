...
export class Documents extends APIResource {
  ...
  retrieve(
    ...
    return this._client.get(path`/v1/documents/${documentID}`, { query, ...options });
  }
  ...
  update(
    ...
    return this._client.patch(path`/v1/documents/${documentID}`, { body, ...options });
  }
  ...
  replaceContent(
    ...
    return this._client.post(path`/v1/documents/${documentID}/content`, { body, ...options });
  }
}
...
/**
 * A document's metadata, its default tab's body, its other tabs, its comments, and
 * the change-detection token for it.
 */
export interface DocumentRetrieveResponse {
  ...
  /**
   * The default tab's body as markdown. Empty string for a document with no content
   * — never absent, so you can read it without a presence check.
   */
...
export namespace DocumentRetrieveResponse {
  ...
  export interface AdditionalTab {
    ...
    /**
     * The tab's title as authored. Not unique within a document, and a placeholder
     * when the tab was never titled.
     */
    title: string;
  }

  /**
   * One review comment, mapped from upstream's open, unvalidated shape into a closed
   * contract.
   *
   * Upstream's own comment schema — `body`, `quoted_text`, `author`, `replies`,
   * `resolved` — is documented as not a closed contract. Only `body` and
   * `quoted_text` are mapped through, as `content` and `context`; commenter
   * identity, reply threads and resolution state are dropped here, not relayed.
   */
  export interface Comment {
    ...
    /**
     * Text the comment was left on. `null` for a comment on no specific text, never an
     * empty string.
     */
    context?: string | null;
  }
...
/**
 * The document after its content was replaced, plus the change-detection token and
 * how much landed.
 */
...
export interface DocumentRetrieveParams {
  /**
   * ID of the organization that owns the document. Required — Profound API keys are
  ...
  /**
   * Include the document's review comments, each mapped to `{content, context}`. On
  ...
  /**
   * Include the document's other tabs. On by default — a tab body is part of the
...
export interface DocumentUpdateParams {
  ...
  /**
   * New title for the document. Renaming sets a permanent lock: once a document is
...
export interface DocumentListParams {
  /**
   * ID of the organization whose documents to list. Required. The caller must be a
  ...
  /**
   * Filter to documents whose name contains this text, case-insensitively. Matches
...
export interface DocumentReplaceContentParams {
  /**
   * New markdown body for the document, replacing everything it held before. An
