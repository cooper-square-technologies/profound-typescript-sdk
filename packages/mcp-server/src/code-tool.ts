...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { PROFOUND_API_KEY: requireValue(readEnv('PROFOUND_API_KEY') ?? client.apiKey, 'set PROFOUND_API_KEY environment variable or provide apiKey client option'), PROFOUND_BASE_URL: readEnv('PROFOUND_BASE_URL') ?? client.baseURL ?? undefined };
